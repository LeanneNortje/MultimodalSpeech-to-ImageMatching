...
#_________________________________________________________________________________________________
#
# This script lists all possible image pairs in order to get the distances between each image and 
...
def main():
    ...
    episode_dict = generate_unimodal_image_episodes.read_in_episodes(args.episodes_fn)
    ...
    for episode in episode_dict:
        ...
        for i, query_key in enumerate(cur_query["keys"]):
            ...
            for j, im_key in enumerate(curr_support_set["keys"]):
                cur_label = curr_support_set["labels"][int(j/K)]
                if cur_label == 'z': continue
                key_file.write(query_key + " " + im_key + "\n")
